...
def rescale_warm_hot_dimension(batch):
    ...
    rescaled = np.zeros_like(cold, dtype="int64")

    # Cold only
    rescaled[(cold == 1) & (warm == 0)] = 1

    # Warm only
    rescaled[(warm == 1) & (cold == 0)] = 2
...
def heuristic_filter_hf(dataset):
    ...
    final_dataset = concatenate_datasets([filtered_either_ds, other_ds]).shuffle(
        seed=42
    )
