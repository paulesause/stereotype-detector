...
from racism_classifier.racism_classifier.preprocessing import heuristic_filter
from racism_classifier.racism_classifier.preprocessing import rescale_warm_hot_dimension, tokenize
...
def apply_lexical_diversity(example):
    return {"lex_div": lexical_diversity(example["labels"])}

a = [{"lex_div": lexical_diversity(text)} for text in data["text"]]
def heuristic_filter_hf(dataset):
    ...
    either_ds = dataset.filter(lambda x: x["label"] == 0)
    other_ds = dataset.filter(lambda x: x["label"] != 0)
